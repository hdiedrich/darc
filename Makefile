...
html:
	...
	@sed -E -e 's/url\(\.\.\/\.\.\/etc/url(..\/etc/g' -i .bak doc/BENCHMARKS.html
	@sed -E -e 's/src=etc/src=..\/etc/g' -i .bak doc/BENCHMARKS.html
	@lua etc/markdown.lua --style etc/css/style.css --inline-style doc/IMPLEMENTATION.md
	@sed -E -e 's/url\(\.\.\/\.\.\/etc/url(..\/etc/g' -i .bak doc/IMPLEMENTATION.html
	@sed -E -e 's/src=etc/src=..\/etc/g' -i .bak doc/IMPLEMENTATION.html
